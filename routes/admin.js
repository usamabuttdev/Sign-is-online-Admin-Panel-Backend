...
router.post('/trainers', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await db.query(
      'INSERT INTO trainers (name, email, phone, specialization, isactive, createdat, updatedat) VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING *',
      [name, email || null, phone || null, specialization || null, isActive !== undefined ? isActive : true]
    );
...
router.put('/trainers/:id', authenticateToken, async (req, res) => {
  try {
    const { name, email, phone, specialization, isActive } = req.body;
    const result = await db.query(
      'UPDATE trainers SET name = COALESCE($1, name), email = COALESCE($2, email), phone = COALESCE($3, phone), specialization = COALESCE($4, specialization), isactive = COALESCE($5, isactive), updatedat = CURRENT_TIMESTAMP WHERE id = $6 RETURNING *',
      [name || null, email || null, phone || null, specialization || null, isActive !== undefined ? isActive : null, req.params.id]
    );
